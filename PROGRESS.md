...
- [ ] (v2.2 revision) Confirm the Supabase project name
      "the-corporate-sustainability" with the builder, then create it via
      MCP at the start of the session.
- [ ] (v2.2 revision) Build the `submissions` table and its RLS policy
      (anon insert-only, no select, update, or delete).
- [ ] (v2.2 revision) Build the two private Storage buckets (EcoVadis
      scorecard file, completed template upload) and their RLS, naming them
      during setup since the spec does not name them.
- [ ] (v2.2 revision) Create `docs/supabase-setup.md`.
- [ ] (v2.2 revision) Build View 2, Company and Contact Details, and wire it
      ahead of Views 3 and 4 on both doors.
- [ ] (v2.2 revision) Update Views 3, 5, and 6 (renumbered 3, 5, 6 in the new
      spec) to write to `submissions` and Storage on submit, including the
      new Rule 5 write-failure handling.
- [ ] (v2.2 revision) Update the confirmation view's copy per spec Section 8
      to reflect that a record now genuinely exists.
- [ ] (v2.2 revision) Add the GDPR consent checkbox and data statement to
      View 2, on both doors.
- [ ] (v2.2 revision) Local test pass across all three submission paths,
      confirming rows and files appear correctly and that the anon key
      cannot read, update, or delete.
- [ ] (v2.2 revision) Run the em dash sweep (criterion 18) across the new
      and updated views.
- [ ] (v2.2 revision) Acceptance criteria pass, verifying all 20 criteria in
      spec Section 13.
- [ ] (v2.2 revision) Set `SUPABASE_URL` and `SUPABASE_ANON_KEY` as Netlify
      environment variables and redeploy via MCP.
...
- Spec revised to v2.2 on 17 September 2026, CLAUDE.md regenerated by
  Project Governor. Data model moves D2 to D3, tier moves 1 to 2: this
  reverses the GDPR line above (v2.2's GDPR outcome is now Applies, with a
  placeholder data statement and deletion contact pending real values before
  real suppliers use the tool) and the line about storing nothing, above.
- The v2.2 spec does not name the two Supabase Storage buckets it requires;
  Claude Code chooses the names during setup and records them in
